...
from .generator_layers import TransformerGenerator, SmoothCondition
...
class Generator(BaseModel):
    def __init__(self, code_num, hidden_dim, attention_dim, max_len, device=None):
        ...
        self.noise_dim = code_num  # Khác GRU, input Transformer là [B, code_num]
        self.transformer = TransformerGenerator(code_num, hidden_dim, max_len, device)
        self.smooth_condition = SmoothCondition(code_num, attention_dim)

    def forward(self, target_codes, lens, noise):
        samples, hiddens = self.transformer(noise)
